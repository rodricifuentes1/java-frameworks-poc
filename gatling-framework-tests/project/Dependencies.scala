...
object Versions {
  ...
  lazy val gatlingVersion: String = "2.1.7"
...
object Dependencies {
  ...
  private[Dependencies] object CompileModules {
    ...
    lazy val gatling: ModuleID = "io.gatling.highcharts" % "gatling-charts-highcharts" % Versions.gatlingVersion
  ...
  private[Dependencies] val core: Seq[ModuleID] = Seq(C.gatling)
  private[Dependencies] val test: Seq[ModuleID] = Seq(T.specs2, T.specs2Scalaz)

  val all: Seq[ModuleID] = utils ++ functional ++ core ++ test
}
